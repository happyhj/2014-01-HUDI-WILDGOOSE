...
" "+className};var removeClass_common=function(DOM,className){if(!this.hasClass(DOM,className))return;if(DOM.className==className){DOM.className="";return}if(this.hasClass(DOM," "+className))DOM.className=DOM.className.replace(" "+className,"");else DOM.className=DOM.className.replace(className+" ","")};var Util={dom:{hasClass:function(){if(isClassListExist)return function(DOM,className){return DOM.classList.contains(className)};else return hasClass_common}(),addClass:function(){if(isClassListExist)return function(DOM,
className){return DOM.classList.add(className)};else return addClass_common}(),removeClass:function(){if(isClassListExist)return function(DOM,className){return DOM.classList.remove(className)};else return addClass_common}()},string:{trim:function(){if("trim"in String.prototype)return function(str){return String.prototype.trim.call(str)};else return function(str){return this.ltrim(this.rtrim(str))}}(),rtrim:function(str){if("trimRight"in String.prototype)return function(str){return String.prototype.trimRight.call(str)};
else return function(str){return str.replace(/\s*$/,"")}}(),ltrim:function(str){if("trimLeft"in String.prototype)return function(str){return String.prototype.trimLeft.call(str)};else return function(str){return str.replace(/^\s*/,"")}}()},template:{compiler:function(dataObj,templateString){var resultStr=Util.string.trim(templateString);for(var variableName in dataObj)if(dataObj[variableName]===0||dataObj[variableName])resultStr=resultStr.replace("\x3c%\x3d "+variableName+" %\x3e",dataObj[variableName]);
return resultStr},getCompiler:function(){return this.compiler},get:function(orgs){var Ajax=CAGE.ajax;var url=orgs.url;var template=null;Ajax.GET({"url":url,"isAsync":false,"callback":function(templateResponse){template=JSON.parse(templateResponse)["data"]["template"]}});return template}}};CAGE.util=Util;if(typeof module!=="undefined"&&module.exports)module.exports=CAGE;else window.CAGE=CAGE})(this);

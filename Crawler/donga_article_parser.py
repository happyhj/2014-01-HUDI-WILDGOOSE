...
def getArticleURLs(article_list_URL) :
...
def extract_contents(html_doc) :
	...
	articlePhotos = article_soup.findAll("div","articlePhotoC")
	if(articlePhotos is not None):
...
def extract_author(html_doc) :
	...
	for idx, text_element_item in enumerate(content_text_element_list_r):
		author_keyword = isNameOccured(text_element_item)
